...
predict.TIRTfit <- function(object, ...) {
  ...
  if (inherits(fit, "stanfit")) {
    out <- .predict_stan(fit, traits, ...)
  } else if (inherits(fit, "mplusObjectTIRT")) {
    out <- .predict_mplus(fit, traits, ...)
  } else if (inherits(fit, "lavaan")) {
    out <- .predict_lavaan(fit, traits, ...)
  }
  out
}

# post process Stan objects
.predict_stan <- function(fit, traits, ...) {
...
.predict_mplus <- function(fit, traits, ...) {
...
.predict_lavaan <- function(fit, traits, ...) {
